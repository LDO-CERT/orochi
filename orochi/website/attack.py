...
def load_mitre_data():
    ...
    try:
        raw_json = gzip.decompress(base64.b85decode(_EMBEDDED_MITRE_B85)).decode(
            "utf-8"
        )
...
def get_all_technique_choices():
    ...
    data = load_mitre_data()
    items = []
    for t_id, meta in sorted(data.items()):
        items.append({"id": t_id, "label": f"{t_id} - {meta.get('name', '')}"})
    return items
...
def get_case_attack_coverage(findings):
    ...
    for t_id, info in sorted(
        by_technique.items(), key=lambda x: (-x[1]["max_score"], x[0])
    ):
        placed = False
