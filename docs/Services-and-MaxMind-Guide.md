...
Once configured:
1. Open any Case in Orochi (`/cases/`).
2. Click **Generate Report** on the case detail screen.
3. Select an executive report template and toggle **Generate AI Summary with Ollama** to `true`.
4. Orochi compiles all logged findings, severity rankings, and MITRE ATT&CK techniques, prompts Ollama via `/api/generate`, and injects the synthesized report into the rendered document.

##### Data Flow & Template Context Variables
When generating an automated report (`POST /case_report/<id>`), Orochi passes the following structured variables to the selected `ReportTemplate`:

| Context Variable | Type | Description |
| :--- | :--- | :--- |
| `case` | `Case` object | The case entity (name, description, author, creation timestamp). |
| `findings` | `QuerySet[Finding]` | All findings logged under the case, including `severity`, `note`, `mitre_attack_technique`, `tags`. |
| `evidences` | `QuerySet[Evidence]` | Associated memory dumps and disk artifacts. |
| `timeline_events`| `QuerySet[TimelineEvent]` | Chronological forensic timeline milestones. |
| `ai_summary` | `str` | Raw markdown output generated by the local Ollama LLM. |
| `ai_summary_html` | `str` | Pre-rendered HTML converted from markdown using `marko`. |

##### Sample Custom Report Template
Admins can upload customized HTML/Jinja templates via **Report Templates** in the admin or cases interface:
...
<head>
  <meta charset="UTF-8">
  <title>Incident Report - {{ case.name }}</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 2rem; color: #1e293b; }
    .header { border-bottom: 2px solid #3b82f6; padding-bottom: 1rem; margin-bottom: 1.5rem; }
    .ai-box { background: #f8fafc; border-left: 4px solid #3b82f6; padding: 1.25rem; border-radius: 0.5rem; margin: 1.5rem 0; }
    .finding-badge { font-weight: bold; padding: 2px 8px; border-radius: 4px; font-size: 0.85em; }
...
<body>
  <div class="header">
    <h1>Incident Report: {{ case.name }}</h1>
    <p><strong>Date:</strong> {{ case.created_at|date:"Y-m-d H:i:s" }} | <strong>Lead Analyst:</strong> {{ case.user.username }}</p>
  </div>

  <h2>1. AI Executive Summary (Ollama)</h2>
  <div class="ai-box">
    {% if ai_summary_html %}
    ...
    {% else %}
      <em>No AI executive summary generated.</em>
    {% endif %}
  </div>

  <h2>2. Detailed Forensic Findings</h2>
  <ul>
    {% for f in findings %}
      <li>
        <span class="finding-badge severity-{{ f.severity }}">{{ f.severity }}</span>
        <strong>{{ f.mitre_attack_technique|default:"Observation" }}:</strong> {{ f.note }}
      </li>
    {% empty %}
      <li>No findings logged for this case.</li>
    {% endfor %}
  </ul>
</body>
</html>
...
##### Air-Gapped Inference & Chain of Custody
- **Zero Data Leakage:** Inference is performed strictly via local HTTP within the Docker Compose network (`http://ollama:11434`). Prompts never transit third-party cloud APIs.
- **Auditable Prompts:** The prompt strictly incorporates structured incident facts (`case.name`, `findings.severity`, `findings.mitre_attack_technique`, and `findings.note`), guaranteeing forensic reproducibility.
